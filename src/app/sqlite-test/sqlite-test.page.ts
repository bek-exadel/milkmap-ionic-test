...
@Component({
  selector: 'sqlite-test-page',
  templateUrl: './sqlite-test.page.html',
  styleUrls: ['./sqlite-test.page.scss'],
})
export class SqliteTestPage implements OnInit, AfterViewInit {
  ...
  async runTest(): Promise<void> {
    try {
      ...
      // let db = await this._sqlite.createConnection("myDB", false, "no-encryption", 1);
      let db = await this._sqlite.createConnection("demo", false, "no-encryption", 1);
